...
#boot{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#F4F3F0;z-index:999}
#app{display:none}
...
<div id="boot" class="grid-bg">
  <div style="width:520px;padding:32px;border:1px solid #CECBC4;background:#ECEAE6">
    <div style="font-size:11px;font-weight:700;color:#C8231A;letter-spacing:0.2em;margin-bottom:24px">AI.5 // BRAND CREATIVE INTELLIGENCE</div>
    <div id="boot-lines"></div>
    <span id="boot-cursor" style="font-size:10px;color:#C8231A;animation:blink 1s step-end infinite">&#x2588;</span>
  </div>
</div>
...
document.addEventListener("DOMContentLoaded",function(){
  var el=document.getElementById("boot-lines");
  var cursor=document.getElementById("boot-cursor");
  // Click boot screen to skip
  document.getElementById("boot").addEventListener("click", launchApp);
  // Safety fallback — always launches after 4s no matter what
  var safetyTimer=setTimeout(launchApp, 4000);
  var i=0;
  var status=["OK","READY","ACTIVE","ONLINE","ENABLED"];
  function next(){
    if(i>=BOOT.length){
    ...
    div.style.cssText="font-size:10px;letter-spacing:0.04em;line-height:2";
    var sw=status.find(function(w){return line.includes(w);});
    var sp=line.includes("SECRET")||line.includes("INITIALISING")||line.startsWith("-");
    if(sw){
      var before=line.substring(0,line.indexOf(sw));
      div.innerHTML='<span style="color:#999590">'+before+'</span><span style="color:#1A7A3C">'+sw+'</span>';
    } else {
      div.innerHTML='<span style="color:'+(sp?"#111110":"#555350")+'">'+line+'</span>';
    }
    el.appendChild(div);
    setTimeout(next,180);
  }
