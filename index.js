...
function downloadMoodboard() {
  ...
  if (!d) return;
  var camps = d.campaigns || [];
  var W = 1200, col = '#F4F3F0', ink = '#111110', mid = '#555350', dim = '#999590', red = '#C8231A', line = '#CECBC4';
  var rowH = 160, headerH = 200, footerH = 60;
  var totalH = headerH + (camps.length * rowH) + footerH;

  var s = '';
  s += '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + totalH + '" viewBox="0 0 ' + W + ' ' + totalH + '">';
  s += '<defs>';
  s += '<style>text{font-family:IBM Plex Mono,Courier New,monospace}';
  s += '.lbl{font-size:9px;fill:' + dim + ';letter-spacing:2px;text-transform:uppercase}';
  s += '.val{font-size:11px;fill:' + mid + '}';
  s += '.ttl{font-size:13px;font-weight:bold;fill:' + ink + '}';
  s += '.yr{font-size:11px;font-weight:bold;fill:' + red + '}';
  s += '</style></defs>';

  // Background
  s += '<rect width="' + W + '" height="' + totalH + '" fill="' + col + '"/>';

  // Grid lines
  for (var gx = 0; gx < W; gx += 32) s += '<line x1="' + gx + '" y1="0" x2="' + gx + '" y2="' + totalH + '" stroke="' + line + '" stroke-width=".4" opacity=".5"/>';
  for (var gy = 0; gy < totalH; gy += 32) s += '<line x1="0" y1="' + gy + '" x2="' + W + '" y2="' + gy + '" stroke="' + line + '" stroke-width=".4" opacity=".5"/>';

  // Header bar
  s += '<rect x="0" y="0" width="' + W + '" height="4" fill="' + ink + '"/>';
  s += '<rect x="0" y="4" width="' + W + '" height="' + (headerH-4) + '" fill="#ECEAE6"/>';

  // Brand name
  s += '<text x="40" y="70" font-size="42" font-weight="bold" fill="' + ink + '" letter-spacing="-1">' + xmlEsc(d.brand||"") + '</text>';
  s += '<text x="40" y="92" font-size="9" fill="' + red + '" letter-spacing="3">BRAND CREATIVE INTELLIGENCE // AI.5</text>';
  ...
  metas.forEach(function(m, mi) {
    var mx = 40 + (mi * 220);
    s += '<text x="' + mx + '" y="130" class="lbl">' + xmlEsc(m[0]) + '</text>';
    s += '<text x="' + mx + '" y="148" class="val">' + xmlEsc(m[1]) + '</text>';
  });

  // Tagline
  if (d.tagline) {
    s += '<text x="40" y="178" font-size="11" font-style="italic" fill="' + mid + '">"' + xmlEsc(d.tagline) + '"</text>';
  }

  // Header bottom border
  s += '<rect x="0" y="' + (headerH-1) + '" width="' + W + '" height="1" fill="' + line + '"/>';

  // Column headers
  var cols = [{x:40,w:40,lbl:'REF'},{x:90,w:300,lbl:'CAMPAIGN'},{x:400,w:80,lbl:'FORMAT'},{x:490,w:80,lbl:'YEAR'},{x:580,w:100,lbl:'AGENCY'},{x:690,w:200,lbl:'PALETTE'},{x:900,w:260,lbl:'TONE / MOOD'}];
  var chY = headerH + 28;
  s += '<rect x="0" y="' + headerH + '" width="' + W + '" height="40" fill="#E2E0DB"/>';
  cols.forEach(function(c) {
    s += '<text x="' + c.x + '" y="' + chY + '" class="lbl">' + c.lbl + '</text>';
  });
  s += '<rect x="0" y="' + (headerH+40) + '" width="' + W + '" height="1" fill="' + line + '"/>';

  // Campaign rows
  camps.forEach(function(c, ci) {
    var ry = headerH + 40 + (ci * rowH);
    var bg = ci % 2 === 0 ? col : '#ECEAE6';
    s += '<rect x="0" y="' + ry + '" width="' + W + '" height="' + rowH + '" fill="' + bg + '"/>';

    var cy = ry + 36;
    // Ref
    s += '<text x="40" y="' + cy + '" class="yr">' + String(ci+1).padStart(2,'0') + '</text>';
    // Title
    var title = (c.title||'').substring(0,35);
    s += '<text x="90" y="' + cy + '" class="ttl">' + xmlEsc(title) + '</text>';
    if (c.agency && c.agency !== 'Unknown') {
      s += '<text x="90" y="' + (cy+18) + '" font-size="9" fill="' + dim + '">' + xmlEsc(c.agency.substring(0,35)) + '</text>';
    }
    // Description
    var desc = (c.description||'').substring(0,80);
    if (desc.length === 80) desc += '...';
    s += '<text x="90" y="' + (cy+42) + '" font-size="9" fill="' + mid + '">' + xmlEsc(desc.substring(0,60)) + '</text>';
    if (desc.length > 60) s += '<text x="90" y="' + (cy+56) + '" font-size="9" fill="' + mid + '">' + xmlEsc(desc.substring(60)) + '</text>';

    // Format
    s += '<text x="400" y="' + cy + '" font-size="9" font-weight="bold" fill="' + ink + '">' + xmlEsc(c.type||'') + '</text>';
    // Year
    s += '<text x="490" y="' + cy + '" class="yr">' + xmlEsc(String(c.year||'')) + '</text>';
    // Awards
    if (c.awards && c.awards !== 'none') {
      s += '<text x="490" y="' + (cy+18) + '" font-size="8" fill="' + red + '">' + xmlEsc((c.awards||'').substring(0,20)) + '</text>';
    }
    // Agency
    s += '<text x="580" y="' + cy + '" font-size="9" fill="' + mid + '">' + xmlEsc((c.agency||'').substring(0,18)) + '</text>';

    // Palette swatches
    (c.palette||[]).forEach(function(col2, pi) {
      if (!col2 || col2 === '#hex') return;
      var px = 690 + (pi * 52);
      s += '<rect x="' + px + '" y="' + (ry+20) + '" width="40" height="40" fill="' + xmlEsc(col2) + '" rx="2"/>';
      s += '<text x="' + px + '" y="' + (ry+76) + '" font-size="7" fill="' + dim + '">' + xmlEsc(col2) + '</text>';
    });

    // Mood
    s += '<text x="900" y="' + cy + '" font-size="9" fill="' + mid + '">' + xmlEsc((c.mood||'').substring(0,30)) + '</text>';
    if (c.visualStyle) {
      var vs = (c.visualStyle||'').substring(0,60);
      s += '<text x="900" y="' + (cy+18) + '" font-size="8" fill="' + dim + '" font-style="italic">' + xmlEsc(vs.substring(0,50)) + '</text>';
    }

    // Row border
    s += '<rect x="0" y="' + (ry+rowH-1) + '" width="' + W + '" height="1" fill="' + line + '"/>';
  });

  // Footer
  var fy = headerH + 40 + (camps.length * rowH);
  s += '<rect x="0" y="' + fy + '" width="' + W + '" height="' + footerH + '" fill="' + ink + '"/>';
  s += '<text x="40" y="' + (fy+22) + '" font-size="8" fill="#F4F3F0" letter-spacing="2">AI.5 // BRAND CREATIVE INTELLIGENCE</text>';
  s += '<text x="40" y="' + (fy+38) + '" font-size="8" fill="#555555" letter-spacing="1">Generated ' + new Date().toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'}) + ' -- ai5.intelligence</text>';
  s += '<text x="' + (W-40) + '" y="' + (fy+30) + '" font-size="8" fill="#555555" text-anchor="end">' + xmlEsc((d.brand||'').toUpperCase()) + ' // CLASSIFIED</text>';
