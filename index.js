...
async function runBrand() {
  ...
  try {
    ...
    if (cat) prompt += ' (' + cat + ')';
    prompt += '. Also search for the brand logo SVG or PNG URL. Return ONLY this raw JSON: {"brand":"","category":"","tagline":"","headquarterCountry":"","brandDomain":"","logoUrl":"","appeal":[""],"creativeThemes":[""],"campaigns":[{"title":"","year":2023,"type":"TV","market":"","description":"","mood":"","agency":"","awards":"","reach":"","visualStyle":"","palette":["#hex"],"links":[{"type":"TVC","label":"","url":""}]}],"competitiveContext":"","recentDirection":""}. Include 4-7 real campaigns. Only include link objects where you found a real URL.';
...
function drawProfile() {
  ...
  if (logoSrc) {
    ...
    h += '<span style="font-size:8px;color:#999590;letter-spacing:.12em;text-transform:uppercase">LOGO ASSET &#8599;</span></a>';
  }
  h += '</div></div>';
  ...
  h += '<button onclick="downloadMoodboard()" style="padding:8px 14px;background:#111110;color:#F4F3F0;font-size:8px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;border:none;cursor:pointer;font-family:inherit">&#8595; DOWNLOAD MOODBOARD</button>';
  h += '</div></div>';
  ...
  shown.forEach(function(c, i) {
    ...
    if (c.agency && c.agency !== "Unknown") {
      h += '<div style="font-size:9px;color:#999590">' + esc(c.agency) + '</div>';
    }
    h += '</div>';
...
function downloadMoodboard() {
  ...
  if (!d) return;
  var camps = d.campaigns || [];
  var W = 1200, rowH = 148, hdrH = 190, ftrH = 50;
  var totalH = hdrH + 40 + (camps.length * rowH) + ftrH;
  var f = "IBM Plex Mono,Courier New,monospace";
  var rows = [];
  var p = function(s) { rows.push(s); };

  p('<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + totalH + '" viewBox="0 0 ' + W + ' ' + totalH + '">');
  p('<rect width="' + W + '" height="' + totalH + '" fill="#F4F3F0"/>');

  for (var gx = 0; gx < W; gx += 32) p('<line x1="' + gx + '" y1="0" x2="' + gx + '" y2="' + totalH + '" stroke="#CECBC4" stroke-width=".4" opacity=".4"/>');
  for (var gy = 0; gy < totalH; gy += 32) p('<line x1="0" y1="' + gy + '" x2="' + W + '" y2="' + gy + '" stroke="#CECBC4" stroke-width=".4" opacity=".4"/>');

  p('<rect x="0" y="0" width="' + W + '" height="4" fill="#111110"/>');
  p('<rect x="0" y="4" width="' + W + '" height="' + (hdrH-4) + '" fill="#ECEAE6"/>');
  p('<text x="40" y="68" font-family="' + f + '" font-size="44" font-weight="bold" fill="#111110">' + xe(d.brand||"") + '</text>');
  p('<text x="40" y="90" font-family="' + f + '" font-size="8" fill="#C8231A" letter-spacing="3">BRAND CREATIVE INTELLIGENCE // AI.5</text>');

  var metas = [["CATEGORY", d.category||""], ["MARKET", d.headquarterCountry||""], ["CAMPAIGNS", camps.length + " ON FILE"], ["DATE", new Date().toISOString().slice(0,10)]];
  metas.forEach(function(m, mi) {
    var mx = 40 + (mi * 220);
    p('<text x="' + mx + '" y="128" font-family="' + f + '" font-size="8" fill="#999590" letter-spacing="2">' + xe(m[0]) + '</text>');
    p('<text x="' + mx + '" y="146" font-family="' + f + '" font-size="11" fill="#555350">' + xe(m[1]) + '</text>');
  });

  if (d.tagline) p('<text x="40" y="174" font-family="' + f + '" font-size="11" font-style="italic" fill="#555350">' + xe(d.tagline) + '</text>');

  p('<rect x="0" y="' + (hdrH-1) + '" width="' + W + '" height="1" fill="#CECBC4"/>');
  p('<rect x="0" y="' + hdrH + '" width="' + W + '" height="40" fill="#E2E0DB"/>');

  var hdrs = [[40,"REF"],[88,"CAMPAIGN"],[390,"FORMAT"],[480,"YEAR"],[570,"AGENCY"],[680,"PALETTE"],[880,"TONE"]];
  hdrs.forEach(function(hd) {
    p('<text x="' + hd[0] + '" y="' + (hdrH+26) + '" font-family="' + f + '" font-size="8" fill="#999590" letter-spacing="2">' + hd[1] + '</text>');
  });
  p('<rect x="0" y="' + (hdrH+40) + '" width="' + W + '" height="1" fill="#CECBC4"/>');

  camps.forEach(function(c, ci) {
    var ry = hdrH + 40 + (ci * rowH);
    var rbg = ci % 2 === 0 ? "#F4F3F0" : "#ECEAE6";
    p('<rect x="0" y="' + ry + '" width="' + W + '" height="' + rowH + '" fill="' + rbg + '"/>');
    var ty = ry + 34;
    p('<text x="40" y="' + ty + '" font-family="' + f + '" font-size="11" font-weight="bold" fill="#C8231A">' + String(ci+1).padStart(2,"0") + '</text>');
    p('<text x="88" y="' + ty + '" font-family="' + f + '" font-size="13" font-weight="bold" fill="#111110">' + xe((c.title||"").substring(0,36)) + '</text>');
    if (c.agency && c.agency !== "Unknown") p('<text x="88" y="' + (ty+17) + '" font-family="' + f + '" font-size="9" fill="#999590">' + xe(c.agency.substring(0,36)) + '</text>');
    var desc = (c.description||"").substring(0,110);
    p('<text x="88" y="' + (ty+40) + '" font-family="' + f + '" font-size="9" fill="#555350">' + xe(desc.substring(0,62)) + '</text>');
    if (desc.length > 62) p('<text x="88" y="' + (ty+54) + '" font-family="' + f + '" font-size="9" fill="#555350">' + xe(desc.substring(62)) + '</text>');
    p('<text x="390" y="' + ty + '" font-family="' + f + '" font-size="9" font-weight="bold" fill="#111110">' + xe(c.type||"") + '</text>');
    p('<text x="480" y="' + ty + '" font-family="' + f + '" font-size="11" font-weight="bold" fill="#C8231A">' + xe(String(c.year||"")) + '</text>');
    if (c.awards && c.awards !== "none") p('<text x="480" y="' + (ty+17) + '" font-family="' + f + '" font-size="8" fill="#C8231A">' + xe((c.awards||"").substring(0,22)) + '</text>');
    p('<text x="570" y="' + ty + '" font-family="' + f + '" font-size="9" fill="#555350">' + xe((c.agency||"").substring(0,18)) + '</text>');
    (c.palette||[]).forEach(function(pc, pi) {
      if (!pc || pc === "#hex") return;
      var px = 680 + (pi * 56);
      p('<rect x="' + px + '" y="' + (ry+16) + '" width="44" height="44" fill="' + xe(pc) + '" rx="2"/>');
      p('<text x="' + px + '" y="' + (ry+74) + '" font-family="' + f + '" font-size="7" fill="#999590">' + xe(pc) + '</text>');
    });
    p('<text x="880" y="' + ty + '" font-family="' + f + '" font-size="9" fill="#555350">' + xe((c.mood||"").substring(0,32)) + '</text>');
    if (c.visualStyle) p('<text x="880" y="' + (ty+17) + '" font-family="' + f + '" font-size="8" font-style="italic" fill="#999590">' + xe((c.visualStyle||"").substring(0,40)) + '</text>');
    p('<rect x="0" y="' + (ry+rowH-1) + '" width="' + W + '" height="1" fill="#CECBC4"/>');
  });

  var fy = hdrH + 40 + (camps.length * rowH);
  p('<rect x="0" y="' + fy + '" width="' + W + '" height="' + ftrH + '" fill="#111110"/>');
  p('<text x="40" y="' + (fy+20) + '" font-family="' + f + '" font-size="8" fill="#F4F3F0" letter-spacing="2">AI.5 // BRAND CREATIVE INTELLIGENCE</text>');
  p('<text x="40" y="' + (fy+36) + '" font-family="' + f + '" font-size="8" fill="#666666">' + new Date().toLocaleDateString("en-GB") + '</text>');
  p('<text x="' + (W-40) + '" y="' + (fy+28) + '" font-family="' + f + '" font-size="9" font-weight="bold" fill="#F4F3F0" text-anchor="end">' + xe((d.brand||"").toUpperCase()) + '</text>');
  p('</svg>');
