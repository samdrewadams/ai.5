...
async function runBrand() {
  ...
  runSteps("ss", ["SEARCHING WEB FOR CAMPAIGNS","ANALYSING CREATIVE STRATEGY","MAPPING COMPETITIVE CONTEXT","COMPILING INTELLIGENCE FILE"]);
  try {
    var prompt = 'Search the web for advertising campaigns and creative strategy of "' + name + '"';
    if (cat) prompt += ' (' + cat + ')';
    prompt += '. Focus on the last 4 years. Return ONLY this raw JSON object: {"brand":"","category":"","tagline":"","headquarterCountry":"","appeal":["","",""],"creativeThemes":["",""],"campaigns":[{"title":"","year":2023,"type":"TV","market":"","description":"","mood":"","agency":"","awards":"","reach":""}],"competitiveContext":"","recentDirection":""}. Include 4-7 real campaigns only.';
...
function drawProfile() {
  ...
  shown.forEach(function(c, i) {
    ...
    h += '</div>';
    if (_openRow === i) {
      h += '<div style="padding:14px 16px 14px 56px;background:#F4F3F0;border-top:1px solid #CECBC4">';
      h += '<p style="font-size:11px;font-weight:300;color:#555350;line-height:1.75;max-width:560px;margin-bottom:14px">' + esc(c.description||"") + '</p>';
      h += '<div style="display:flex;gap:28px">';
      if (c.mood)   h += '<div><div style="font-size:8px;color:#999590;text-transform:uppercase">TONE</div><div style="font-size:10px;margin-top:4px">'+esc(c.mood)+'</div></div>';
      if (c.reach && c.reach !== "unknown") h += '<div><div style="font-size:8px;color:#999590;text-transform:uppercase">REACH</div><div style="font-size:10px;margin-top:4px">'+esc(c.reach)+'</div></div>';
      if (c.awards && c.awards !== "none") h += '<div><div style="font-size:8px;color:#999590;text-transform:uppercase">AWARDS</div><div style="font-size:10px;color:#C8231A;font-weight:500;margin-top:4px">'+esc(c.awards)+'</div></div>';
      h += '</div></div>';
    }
    h += '</div>';
  });
