...
app.post("/api/intelligence", async (req, res) => {
  ...
  try {
    ...
    if (!m) return res.status(500).json({ error: "No JSON found. Got: " + text.substring(0, 200) });
...
.asset-link{display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid #CECBC4;background:#ECEAE6;margin-bottom:6px;text-decoration:none;transition:background .15s}
.asset-link:hover{background:#E2E0DB}
...
var cats = ["Craft Beer","Luxury Fashion","Electric Vehicles","Fintech","Sportswear","Streaming","Beauty","Spirits"];
...
function runSteps(containerId, stepsArr) {
  ...
  function tick() {
    if (cur > 0) {
      var prev = document.getElementById(containerId + "_" + (cur-1));
      if (prev) { prev.style.opacity = "1"; prev.children[0].style.color = "#1A7A3C"; prev.children[0].textContent = "[OK]"; }
    }
    if (cur < stepsArr.length) {
      var el2 = document.getElementById(containerId + "_" + cur);
      if (el2) { el2.style.opacity = "1"; el2.children[0].style.color = "#C8231A"; el2.children[0].textContent = "[>>]"; el2.children[1].style.color = "#111110"; }
...
async function runBrand() {
  ...
  try {
    ...
    if (cat) prompt += ' (' + cat + ')';
    prompt += '. Focus on the last 4 years. Search for the brand\'s official logo as an SVG or vector file URL (check their official website, Wikipedia SVG files, or brand asset pages). For each campaign, search for real URLs to the actual work: YouTube links for TVCs, award pages, campaign websites, social posts, press coverage. Return ONLY this raw JSON object: {"brand":"","category":"","tagline":"","headquarterCountry":"","brandDomain":"example.com","logoUrl":"","appeal":["","",""],"creativeThemes":["",""],"campaigns":[{"title":"","year":2023,"type":"TV","market":"","description":"","mood":"","agency":"","awards":"","reach":"","visualStyle":"","palette":["#hex","#hex","#hex"],"links":[{"type":"TVC","label":"","url":""},{"type":"OOH","label":"","url":""},{"type":"SOCIAL","label":"","url":""},{"type":"PRESS","label":"","url":""}]}],"competitiveContext":"","recentDirection":""}. Include 4-7 real campaigns only. For logoUrl return the most official SVG or PNG URL you can find - if none found leave empty. Only include campaign links where you have found a real URL.';
...
function drawProfile() {
  ...
  var h = "";
  h += '<button onclick="backToSearch()" style="display:inline-flex;align-items:center;gap:8px;margin-bottom:32px;padding:0"><span style="font-size:9px;color:#999590;letter-spacing:.14em;text-transform:uppercase">&#8592; RETURN TO SEARCH</span></button>';
  ...
  if (logoSrc) {
    h += '<img src="' + esc(logoSrc) + '" alt="' + esc(d.brand||"") + ' logo" style="max-width:64px;max-height:64px;object-fit:contain" onerror="logoFallback(this)">';
  } else {
  ...
  if (logoSrc) {
    h += '<a href="' + esc(logoSrc) + '" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;margin-top:6px;text-decoration:none">';
    h += '<span style="font-size:8px;color:#999590;letter-spacing:.12em;text-transform:uppercase">LOGO ASSET &#8599;</span>';
    h += '</a>';
  }
  h += '</div></div>';
  ...
  h += '<div style="display:flex;align-items:center;gap:8px"><span class="led led-g"></span><span style="font-size:9px;color:#1A7A3C;letter-spacing:.14em;text-transform:uppercase">ACTIVE PROFILE</span></div>';
  h += '<span style="font-size:9px;color:#999590">' + esc(d.headquarterCountry||"") + '</span>';
  h += '<button onclick="downloadMoodboard()" style="margin-top:4px;padding:8px 14px;background:#111110;color:#F4F3F0;font-size:8px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;border:none;cursor:pointer;font-family:inherit">&#8595; DOWNLOAD MOODBOARD</button>';
  h += '</div>';
  h += '</div>';
  ...
  types.forEach(function(t) {
    var isOn = t === _activeType;
    h += '<button class="chip' + (isOn ? ' tag' : '') + '" data-type="' + esc(t) + '" onclick="setType(this.dataset.type)">' + esc(t) + '</button>';
  });
  h += '</div></div>';
  ...
  shown.forEach(function(c, i) {
    ...
    if (_openRow === i) {
      ...
      if (c.palette && c.palette.length) {
        ...
        h += '<div style="font-size:8px;color:#999590;letter-spacing:.12em;text-transform:uppercase;margin-right:4px">PALETTE</div>';
        c.palette.forEach(function(col) {
          if (!col || col === '#hex') return;
          h += '<div title="' + esc(col) + '" style="width:24px;height:24px;background:' + esc(col) + ';border:1px solid rgba(0,0,0,.1);flex-shrink:0"></div>';
          h += '<div style="font-size:8px;color:#999590">' + esc(col) + '</div>';
        });
        h += '</div>';
      }
      h += '</div>';
      ...
      // Right: asset links
      h += '<div>';
      var typeIcons = { TVC: '&#9654;', OOH: '&#9632;', SOCIAL: '&#9670;', PRESS: '&#9656;' };
      var typeColors = { TVC: '#C8231A', OOH: '#1A7A3C', SOCIAL: '#555350', PRESS: '#999590' };
      var links = (c.links || []).filter(function(l) { return l && l.url && l.url.length > 4 && l.url.indexOf('http') === 0; });
      if (links.length) {
        links.forEach(function(l) {
          var icon = typeIcons[l.type] || '&#9656;';
          var col = typeColors[l.type] || '#555350';
          h += '<a href="' + esc(l.url) + '" target="_blank" rel="noopener" class="asset-link">';
          h += '<span style="font-size:10px;color:' + col + ';flex-shrink:0">' + icon + '</span>';
          h += '<div style="min-width:0">';
          h += '<div style="font-size:8px;font-weight:700;color:' + col + ';letter-spacing:.12em;text-transform:uppercase">' + esc(l.type) + '</div>';
          h += '<div style="font-size:9px;color:#555350;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(l.label||l.url) + '</div>';
          h += '</div>';
          h += '<span style="margin-left:auto;font-size:9px;color:#CECBC4;flex-shrink:0">&#8599;</span>';
          h += '</a>';
        });
      } else {
        h += '<div style="padding:14px 16px;border:1px solid #CECBC4;background:#ECEAE6;font-size:9px;color:#999590">No asset links retrieved for this campaign.</div>';
      }
      h += '</div>';
...
function downloadMoodboard() {
  ...
  if (!d) return;
  var camps = d.campaigns || [];
  var W = 1200, rowH = 148, headerH = 190, footerH = 50;
  var totalH = headerH + 40 + (camps.length * rowH) + footerH;
  var c0 = '#F4F3F0', c1 = '#ECEAE6', c2 = '#E2E0DB';
  var ink = '#111110', mid = '#555350', dim = '#999590', red = '#C8231A', bdr = '#CECBC4';
  var ff = 'IBM Plex Mono,Courier New,monospace';
  var rows = [];

  rows.push('<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + totalH + '" viewBox="0 0 ' + W + ' ' + totalH + '">');

  // Background
  rows.push('<rect width="' + W + '" height="' + totalH + '" fill="' + c0 + '"/>');

  // Grid
  for (var gx = 0; gx < W; gx += 32) {
    rows.push('<line x1="' + gx + '" y1="0" x2="' + gx + '" y2="' + totalH + '" stroke="' + bdr + '" stroke-width=".4" opacity=".4"/>');
  }
  for (var gy = 0; gy < totalH; gy += 32) {
    rows.push('<line x1="0" y1="' + gy + '" x2="' + W + '" y2="' + gy + '" stroke="' + bdr + '" stroke-width=".4" opacity=".4"/>');
  }

  // Header
  rows.push('<rect x="0" y="0" width="' + W + '" height="4" fill="' + ink + '"/>');
  rows.push('<rect x="0" y="4" width="' + W + '" height="' + (headerH - 4) + '" fill="' + c1 + '"/>');
  rows.push('<text x="40" y="68" font-family="' + ff + '" font-size="44" font-weight="bold" fill="' + ink + '" letter-spacing="-1">' + xmlEsc(d.brand || '') + '</text>');
  rows.push('<text x="40" y="90" font-family="' + ff + '" font-size="8" fill="' + red + '" letter-spacing="3">BRAND CREATIVE INTELLIGENCE // AI.5</text>');

  var metas = [
    ['CATEGORY', d.category || ''],
    ['MARKET', d.headquarterCountry || ''],
    ['CAMPAIGNS', camps.length + ' ON FILE'],
    ['DATE', new Date().toISOString().slice(0, 10)]
  ];
  metas.forEach(function(m, mi) {
    var mx = 40 + (mi * 220);
    rows.push('<text x="' + mx + '" y="128" font-family="' + ff + '" font-size="8" fill="' + dim + '" letter-spacing="2">' + xmlEsc(m[0]) + '</text>');
    rows.push('<text x="' + mx + '" y="146" font-family="' + ff + '" font-size="11" fill="' + mid + '">' + xmlEsc(m[1]) + '</text>');
  });

  if (d.tagline) {
    rows.push('<text x="40" y="174" font-family="' + ff + '" font-size="11" font-style="italic" fill="' + mid + '">' + xmlEsc('"' + d.tagline + '"') + '</text>');
  }

  rows.push('<rect x="0" y="' + (headerH - 1) + '" width="' + W + '" height="1" fill="' + bdr + '"/>');

  // Col headers
  rows.push('<rect x="0" y="' + headerH + '" width="' + W + '" height="40" fill="' + c2 + '"/>');
  var hdrs = [[40,'REF'],[88,'CAMPAIGN'],[390,'FORMAT'],[480,'YEAR'],[570,'AGENCY'],[680,'PALETTE'],[880,'TONE']];
  hdrs.forEach(function(h) {
    rows.push('<text x="' + h[0] + '" y="' + (headerH + 26) + '" font-family="' + ff + '" font-size="8" fill="' + dim + '" letter-spacing="2">' + h[1] + '</text>');
  });
  rows.push('<rect x="0" y="' + (headerH + 40) + '" width="' + W + '" height="1" fill="' + bdr + '"/>');

  // Rows
  camps.forEach(function(c, ci) {
    var ry = headerH + 40 + (ci * rowH);
    var rbg = ci % 2 === 0 ? c0 : c1;
    rows.push('<rect x="0" y="' + ry + '" width="' + W + '" height="' + rowH + '" fill="' + rbg + '"/>');

    var ty = ry + 34;
    rows.push('<text x="40" y="' + ty + '" font-family="' + ff + '" font-size="11" font-weight="bold" fill="' + red + '">' + String(ci + 1).padStart(2, '0') + '</text>');

    var title = xmlEsc((c.title || '').substring(0, 36));
    rows.push('<text x="88" y="' + ty + '" font-family="' + ff + '" font-size="13" font-weight="bold" fill="' + ink + '">' + title + '</text>');

    if (c.agency && c.agency !== 'Unknown') {
      rows.push('<text x="88" y="' + (ty + 17) + '" font-family="' + ff + '" font-size="9" fill="' + dim + '">' + xmlEsc(c.agency.substring(0, 36)) + '</text>');
    }

    var desc = (c.description || '').substring(0, 110);
    rows.push('<text x="88" y="' + (ty + 40) + '" font-family="' + ff + '" font-size="9" fill="' + mid + '">' + xmlEsc(desc.substring(0, 62)) + '</text>');
    if (desc.length > 62) {
      rows.push('<text x="88" y="' + (ty + 54) + '" font-family="' + ff + '" font-size="9" fill="' + mid + '">' + xmlEsc(desc.substring(62)) + '</text>');
    }

    rows.push('<text x="390" y="' + ty + '" font-family="' + ff + '" font-size="9" font-weight="bold" fill="' + ink + '">' + xmlEsc(c.type || '') + '</text>');
    rows.push('<text x="480" y="' + ty + '" font-family="' + ff + '" font-size="11" font-weight="bold" fill="' + red + '">' + xmlEsc(String(c.year || '')) + '</text>');

    if (c.awards && c.awards !== 'none') {
      rows.push('<text x="480" y="' + (ty + 17) + '" font-family="' + ff + '" font-size="8" fill="' + red + '">' + xmlEsc((c.awards || '').substring(0, 22)) + '</text>');
    }

    rows.push('<text x="570" y="' + ty + '" font-family="' + ff + '" font-size="9" fill="' + mid + '">' + xmlEsc((c.agency || '').substring(0, 18)) + '</text>');

    (c.palette || []).forEach(function(pc, pi) {
      if (!pc || pc === '#hex') return;
      var px = 680 + (pi * 56);
      rows.push('<rect x="' + px + '" y="' + (ry + 16) + '" width="44" height="44" fill="' + xmlEsc(pc) + '" rx="2"/>');
      rows.push('<text x="' + px + '" y="' + (ry + 74) + '" font-family="' + ff + '" font-size="7" fill="' + dim + '">' + xmlEsc(pc) + '</text>');
    });

    rows.push('<text x="880" y="' + ty + '" font-family="' + ff + '" font-size="9" fill="' + mid + '">' + xmlEsc((c.mood || '').substring(0, 32)) + '</text>');
    if (c.visualStyle) {
      rows.push('<text x="880" y="' + (ty + 17) + '" font-family="' + ff + '" font-size="8" font-style="italic" fill="' + dim + '">' + xmlEsc((c.visualStyle || '').substring(0, 40)) + '</text>');
    }

    rows.push('<rect x="0" y="' + (ry + rowH - 1) + '" width="' + W + '" height="1" fill="' + bdr + '"/>');
  });

  // Footer
  var fy = headerH + 40 + (camps.length * rowH);
  rows.push('<rect x="0" y="' + fy + '" width="' + W + '" height="' + footerH + '" fill="' + ink + '"/>');
  rows.push('<text x="40" y="' + (fy + 20) + '" font-family="' + ff + '" font-size="8" fill="#F4F3F0" letter-spacing="2">AI.5 // BRAND CREATIVE INTELLIGENCE</text>');
  rows.push('<text x="40" y="' + (fy + 36) + '" font-family="' + ff + '" font-size="8" fill="#666666">' + xmlEsc(new Date().toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'})) + '</text>');
  rows.push('<text x="' + (W - 40) + '" y="' + (fy + 28) + '" font-family="' + ff + '" font-size="9" font-weight="bold" fill="#F4F3F0" text-anchor="end">' + xmlEsc((d.brand || '').toUpperCase()) + '</text>');
  ...
  a.href = url;
  a.download = (d.brand || 'brand').toLowerCase().replace(/\s+/g, '-') + '-moodboard.svg';
...
function logoFallback(img) {
  var initials = (_pd && _pd.brand) ? _pd.brand.substring(0,2).toUpperCase() : '??';
  img.parentNode.innerHTML = '<span style="font-size:18px;font-weight:700;color:#CECBC4">' + initials + '</span>';
}
...
function renderBrief(d) {
  ...
  h += '<div>';
  h += '<div style="font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase">STRATEGIC READ</div>';
...
function renderCat(d) {
  ...
  h += '<div>';
  h += '<div style="font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase">OVERVIEW -- ' + esc((d.category||"").toUpperCase()) + '</div>';
...
(function init(){
  ...
  ["LIVE INTELLIGENCE FEED //","BRAND CREATIVE DATABASE //","REAL-TIME WEB SEARCH //","CLASSIFICATION: UNRESTRICTED //","AI.5 SYSTEM ONLINE //","ALL MARKETS // ALL MEDIA // ALL CATEGORIES //"].forEach(function(t,i,a){
