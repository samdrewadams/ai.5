...
app.post("/api/intelligence", async (req, res) => {
  ...
  try {
    ...
    if (!text) return res.status(500).json({ error: "No text in response. Stop reason: " + stopReason });
    const clean = text.replace(/```json/g, "").replace(/```/g, "").replace(/]*>/g, "").replace(/<\/antml:cite>/g, "").replace(/<cite[^>]*>/g, "").replace(/<\/cite>/g, "").trim();
    const m = clean.match(/\{[\s\S]*\}/);
    if (!m) return res.status(500).json({ error: "No JSON in response. Got: " + text.substring(0, 300) });
    try {
      return res.json(JSON.parse(m[0]));
    } catch(parseErr) {
      return res.status(500).json({ error: "JSON parse failed: " + parseErr.message + ". Raw: " + m[0].substring(0, 200) });
    }
  } catch (err) {
...
function drawProfile() {
  ...
  if (ign) {
    ...
    var barColor = overall >= 80 ? "#0A5C2A" : overall >= 65 ? "#1A7A3C" : overall >= 45 ? "#C8A000" : "#C8231A";
    ...
    (ign.breakdown||[]).forEach(function(b, bi) {
      ...
      var bc = sc >= 80 ? "#0A5C2A" : sc >= 65 ? "#1A7A3C" : sc >= 45 ? "#C8A000" : "#C8231A";
      var isRight = bi % 2 === 1;
      ...
      if (b.evidence) {
        h += '<p style="font-size:9px;font-weight:300;color:#555350;line-height:1.6">' + esc(b.evidence);
        if (b.sourceUrl && b.sourceUrl.indexOf('http') === 0) {
          h += ' <a href="' + esc(b.sourceUrl) + '" target="_blank" rel="noopener" style="font-size:8px;color:#999590;letter-spacing:.08em;text-transform:uppercase;text-decoration:none;border-bottom:1px solid #CECBC4">SOURCE &#8599;</a>';
        }
        h += '</p>';
      }
      h += '</div>';
