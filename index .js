...
async function runBrand() {
  ...
  try {
    ...
    if (cat) prompt += ' (' + cat + ')';
    prompt += '. Focus on the last 4 years. For each campaign, search for real URLs to the actual work: YouTube links for TVCs, award pages, campaign websites, social posts, press coverage. Return ONLY this raw JSON object: {"brand":"","category":"","tagline":"","headquarterCountry":"","appeal":["","",""],"creativeThemes":["",""],"campaigns":[{"title":"","year":2023,"type":"TV","market":"","description":"","mood":"","agency":"","awards":"","reach":"","visualStyle":"","palette":["#hex","#hex","#hex"],"links":[{"type":"TVC","label":"","url":""},{"type":"OOH","label":"","url":""},{"type":"SOCIAL","label":"","url":""},{"type":"PRESS","label":"","url":""}]}],"competitiveContext":"","recentDirection":""}. Include 4-7 real campaigns only. Only include links where you have found a real URL - omit any link object where you cannot find a genuine URL.';
...
function drawProfile() {
  ...
  h += '<div><div style="font-size:10px;font-weight:500;color:#999590;letter-spacing:.2em;text-transform:uppercase;margin-bottom:8px">INTELLIGENCE FILE // ' + esc((d.category||"").toUpperCase()) + '</div>';
  h += '<div style="font-size:26px;font-weight:700;letter-spacing:-.02em">' + esc((d.brand||"").toUpperCase()) + '</div></div>';
  ...
  h += '<span style="font-size:9px;color:#999590">' + esc(d.headquarterCountry||"") + '</span></div></div>';
    ...
    if (_openRow === i) {
      ...
      if (links.length) {
        links.forEach(function(l) {
          ...
          h += '<a href="' + esc(l.url) + '" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid #CECBC4;background:#ECEAE6;margin-bottom:6px;text-decoration:none;transition:background .15s" onmouseover="this.style.background=\'#E2E0DB\'" onmouseout="this.style.background=\'#ECEAE6\'">';
